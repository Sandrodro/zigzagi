...
**Stack:** React 18, TypeScript, Vite, TanStack Query (react-query) for server state, Vitest + Testing Library (no Tailwind yet).
...
- `src/api/play.ts` — TanStack Query hooks for Play endpoints (`useToday`, `useCheckCells`, `useRevealCells`); raw `fetch` wrappers are module-private. `QueryClientProvider` is set up in `src/main.tsx`; component tests must wrap renders in it.
