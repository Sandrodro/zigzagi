...
def load_wordpool(db: Session) -> Wordlist:
    """The active lemma pool — the only wordlist used to fill crosswords."""
    words = db.scalars(select(WordpoolLemma.word).where(WordpoolLemma.status == "active")).all()
    return Wordlist(list(words))


def load_active_wordlist(db: Session) -> Wordlist:
    return load_wordpool(db)
