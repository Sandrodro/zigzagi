...
def test_promote_tick_publishes_due(db_session):
    ...
    db_session.add(Puzzle(id=uuid.uuid4(), live_date=today, grid_template={}, status="scheduled", seed=1, version=1))
    db_session.add(Puzzle(id=uuid.uuid4(), live_date=today + dt.timedelta(days=5), grid_template={}, status="scheduled", seed=1, version=1))
    db_session.flush()
